...
# maxWindowSize = systemInformation['maxWindowSize'] + 1
...
serverCapacities = np.array([400 for s in range(numOfServer)])
...
if __name__ == "__main__":
    ...
    for wsize_index in range(len(windowSizes)):
        ...
        for V in Vs:
            for t in range(mtime):
                ...
                # varOfQueueBacklogs[V][t] = np.var(queueSizes)
    ...
    np.save("resultsPOSCAS_perfect_Cap/capacity400/avgDelays.npy", avgDelays)
    end_time = time()
    np.save("resultsPOSCAS_perfect_Cap/capacity400/runtime.npy", end_time - start_time)
